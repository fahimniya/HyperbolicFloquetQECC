...
def calc_recovery(p_depol, p_depol2 = 0, p_meas = 0, p_em3 = 0, meas_heuristic = False, min_errorbar = 0.01, max_num_shots = 10000):
    time_0 = time.time()
    ...
    while (std_rec+1e-9) / (1-p_rec+1e-9) / log(10) > min_errorbar and len(shots_res) < max_num_shots:
...
num_sites = 72
folder_name = f"../Lattices/Honeycomb/HC{num_sites}"
...
rec_res = Parallel(n_jobs=num_cores)(delayed(calc_recovery)(p_depol=0, p_meas=0, p_em3=p_err) for p_err in p_errs)
